...
const fetchFromTMDB = async <T>(endpoint: string, params: string = ''): Promise<T> => {
...
const processContentItem = (item: any, media_type: 'movie' | 'tv'): ContentItem => ({
  ...
  // Use w500 for general access; components can optimize to w342 for grids
...
export const fetchHeroContent = async (): Promise<ContentItem[]> => {
    ...
    try {
        ...
        if (items.length > 0) {
            try {
                // Enrich first item for immediate display
...
// Return configurations for App.tsx to handle staggered loading
...
export const getGenreList = async (): Promise<Genre[]> => {
    // Check cache first for genres as they rarely change
...
export const searchMulti = async (query: string): Promise<ContentItem[]> => {
    if (!query) return [];
    const data = await fetchFromTMDB<{ results: any[] }>('search/multi', `query=${encodeURIComponent(query)}`);
...
export const searchTVShows = async (query: string): Promise<ContentItem[]> => {
    if (!query) return [];
    const data = await fetchFromTMDB<{ results: any[] }>('search/tv', `query=${encodeURIComponent(query)}`);
    return processContentList(data.results, 'tv');
};
...
export const getStreamingSources = (id: number, media_type: 'movie' | 'tv', season: number = 1, episode: number = 1): StreamingSource[] => {
    ...
    // 1. VidSrc.cc (Main Server)
    const vidSrcCcUrl = media_type === 'tv'
        ? `https://vidsrc.cc/v2/embed/tv/${id}/${season}/${episode}`
        : `https://vidsrc.cc/v2/embed/movie/${id}`;
    sources.push({ name: 'Server 1: VidSrc.cc', url: vidSrcCcUrl });

    // 2. MoviesAPI (MoviesAPI Club)
    const moviesApiUrl = media_type === 'tv'
        ? `https://moviesapi.club/tv/${id}-${season}-${episode}`
        : `https://moviesapi.club/movie/${id}`;
    sources.push({ name: 'Server 2: MoviesAPI', url: moviesApiUrl });
...
export const fetchMovieDownloads = async (query: string): Promise<Torrent[]> => {
    ...
    try {
        ...
        try {
            ...
            // Fail silently or add secondary proxy if available
        }
    }
    return [];
};
