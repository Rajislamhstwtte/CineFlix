...
import { getRecommendations, getSeasonDetails, getSimilarByGenre } from '../services/movieService';
import { PlayIcon, CloseIcon, StarIcon, LoadingSpinner, PlusIcon, CheckIcon, ShareIcon, DownloadIcon, BackIcon } from './IconComponents';
...
const MovieDetail: React.FC<MovieDetailProps> = ({ 
  content, onClose, onSmartPlay, onPlayEpisode, onDownload, isLoading, 
  ...
  const [selectedSeason, setSelectedSeason] = useState<Season | null>(content.seasons ? content.seasons[0] : null);
  ...
  const getPlayButtonText = () => {
    ...
    if (historyItem && historyItem.lastSeason && historyItem.lastEpisode) {
        // If user finished (>95%) the last episode, suggest next one is handled in onSmartPlay
            ...
            <div className="relative h-64 md:h-96 w-full flex-shrink-0">
                ...
                <div className="absolute bottom-0 left-0 p-6 md:p-8 w-full">
                    ...
                    <div className="flex items-center gap-4 text-sm md:text-base font-semibold text-gray-700 dark:text-gray-300">
                        ...
                        {content.number_of_seasons && <span>{content.number_of_seasons} Seasons</span>}
                    </div>
                </div>
            </div>
                        ...
                        <div className="mt-8 border-t border-gray-200 dark:border-gray-800 pt-8">
                             <div className="flex items-center justify-between mb-6">
                                ...
                                <div className="flex space-x-2 overflow-x-auto pb-2 scrollbar-thin max-w-[200px] sm:max-w-md">
                                    {Array.from({ length: content.number_of_seasons || 0 }, (_, i) => i + 1).map(num => (
                                        <button
                                            key={num}
                             ...
                             {isSeasonLoading || !selectedSeason ? (
