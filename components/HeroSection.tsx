  ...
  return (
    ...
    >
      {items.map((item, index) => (
        <div
          key={item.id}
          className={`absolute inset-0 transition-opacity duration-700 ease-in-out will-change-opacity ${index === currentIndex ? 'opacity-100 z-10' : 'opacity-0 z-0'}`}
        >
          <img
            src={item.backdrop_path}
            alt={item.title}
            className={`w-full h-full object-cover transform transition-transform duration-[10000ms] ease-out will-change-transform ${index === currentIndex ? 'scale-110' : 'scale-100'}`}
