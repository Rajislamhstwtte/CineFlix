  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      if (searchContainerRef.current && !searchContainerRef.current.contains(event.target as Node)) {
        // Only collapse if query is empty to prevent accidental closing while typing
        if (!query) {
            setIsExpanded(false);
        }
      }
  ...
  return (
    <div className="relative z-50" ref={searchContainerRef}>
      ...
      {isExpanded && query.length > 1 && (
        <div className="absolute top-full mt-4 right-0 w-60 md:w-80 bg-white/95 dark:bg-gray-900 rounded-2xl border border-black/5 dark:border-gray-800 shadow-2xl z-50 max-h-[60vh] overflow-y-auto scrollbar-thin backdrop-blur-xl animate-fade-in-up ring-1 ring-black/5">
          {isLoading ? (
