...
const MovieCard: React.FC<MovieCardProps> = ({ content, onSelectMovie, onSmartPlay, onDownload }) => {
  ...
  // We use setTimeout(..., 0) to break the long task.
  // This allows the browser to paint the 'active' state (button press) immediately
  // before executing the heavy logic (ads, navigation, state updates).
  ...
  return (
    <div
      className="group relative rounded-lg overflow-hidden cursor-pointer bg-gray-200 dark:bg-gray-900 aspect-[2/3] transform-gpu transition-transform duration-200 hover:scale-[1.03] hover:z-10 shadow-md hover:shadow-xl border border-transparent hover:border-red-500/30"
    ...
    >
      ...
      <div className="absolute top-2 right-2 bg-black/70 text-white text-[10px] font-bold uppercase px-2 py-0.5 rounded border border-white/10 pointer-events-none">
        {content.media_type}
      </div>

      {/* Static Info */}
      <div className="absolute bottom-0 left-0 right-0 p-3 group-hover:opacity-0 transition-opacity duration-300 pointer-events-none">
      ...
      {/* Hover Overlay - Using buttons for interaction */}
      <div className="absolute inset-0 bg-black/80 flex flex-col items-center justify-center p-4 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
        <h4 className="font-bold text-sm text-white text-center mb-4 line-clamp-2 pointer-events-none">{content.title}</h4>
